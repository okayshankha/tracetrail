...
export default function MethodFilterDropdown({ dropdownValues = {} }: { dropdownValues?: { [key: string]: string } }) {
    ...
    return (
        <div className={`col-3 dropdown ${requestLogs.dropdowns.methodFilter.visible ? 'show' : ''}`}>
            ...
            <div className={`dropdown-menu ${requestLogs.dropdowns.methodFilter.visible ? 'show' : ''}`}
                style={{
                    position: 'absolute',
...

                {
                    Object.keys(dropdownValues).map((dropdownKey: string) =>
                        <span key={dropdownKey} className="dropdown-item" style={{ fontSize: '10px' }} onClick={() => SetDropdownValue(dropdownKey)}>{dropdownValues[dropdownKey]}</span>
                    )
                }
            </div>
